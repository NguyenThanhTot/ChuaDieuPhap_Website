...
public class HomePageController {
    ...
    public ResponseEntity<ApiResponse<HomepageDataDTO>> getHomePageData() {
        HomepageDataDTO data = HomepageDataDTO.builder()
                ...
                .notifications(notificationService.findHomepageNotificationsLimited().stream()
                        .map(this::toNotificationDTO).collect(Collectors.toList()))
                .featuredNotifications(notificationService.findFeaturedNotificationsLimited().stream()
                        .map(this::toNotificationDTO).collect(Collectors.toList()))
                .events(eventService.findHomepageEventsLimited().stream()
                        .map(this::toEventDTO).collect(Collectors.toList()))
                .featuredEvents(eventService.findFeaturedEventsLimited().stream()
                        .map(this::toEventDTO).collect(Collectors.toList()))
                .news(newsService.findHomepageNewsLimited().stream()
                        .map(this::toNewsDTO).collect(Collectors.toList()))
                .featuredNews(newsService.findFeaturedNewsLimited().stream()
                        .map(this::toNewsDTO).collect(Collectors.toList()))
                .dharmaTalks(dharmaTalkService.findHomepageDharmaTalksLimited().stream()
